...
<ResponseField name="Errors" type="{ error: true, message }">
  ...
  on sandbox confinement. See [Errors and limits](/errors-and-limits).
</ResponseField>
...
<ResponseField name="End-user isolation" type="subject">
  ...
  that person's rows. `subject` is your own opaque id, accepted on `POST /v1/api-keys`,
  `POST /v1/checkouts` and `POST /v1/buyer-profiles`. A key without one is tenant-wide. See
...
<ResponseField name="Idempotency" type="Idempotency-Key header">
  ...
  a different body is `409`. Keys are namespaced per `(tenant, subject)` and never expire. See
...
[Use Checkoutvia from an LLM agent](/agents) has the same thing shaped as model tools.
</Note>
