...
<ParamField body="subject" type="string">
  Your own id for the end user this profile belongs to. Max 200 characters, opaque to us, never
  sent to a merchant.
...
There is no `?subject=` filter for a tenant-wide key — read `subject` off each row.
...
`subject` is **not** accepted here. Unlike create, this schema is strict.
...
The schema is strict, so an unexpected key is a `400` rather than a silently dropped field. That
is most likely to bite you with `subject`:
