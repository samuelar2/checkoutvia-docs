...
<ParamField body="buyerProfileId" type="string">
  ...
  your tenant — and, on a [subject-bound key](/end-user-isolation), to the same end user — or
  you get `400 buyerProfileId not found`.

  Without one the agent has no address or email, and most checkouts cannot be completed.
</ParamField>

<ParamField body="subject" type="string">
  Your own id for the end user this purchase is for. Max 200 characters, opaque to us, never sent
  to the merchant. Comes back on the checkout when set, and omitted when not.
  ...
  On a tenant-wide key it is how a row gets tagged in the first place.
...
| Response | When |
| --- | --- |
| `201` | First use of the key. A new checkout, a new run. |
| `200` | The key has been used before with the same body. The **original** checkout, in whatever state it is in now — poll it, do not assume `queued`. |
| `409` | The key has been used before with a **different** body. Nothing is created. |
...
and does not launch a second browser. It is checked *after* URL and sandbox validation, so a
replay of a request whose target has since become unreachable can still return `400`.

<Note>
This is idempotency of the **call**, not of the run. Once a checkout is `failed`, its key returns
that failure for as long as the row exists. Retrying for real means a new checkout and a new key.
</Note>
...
<Note>
The summary carries **no `subject`**, and there is no `?subject=` filter. A
[subject-bound key](/end-user-isolation) sees only its own end user's checkouts here, so it does
not need one; a tenant-wide key sees everybody's and cannot tell them apart without fetching each
checkout. Keep your own map from checkout id to user at creation time.
</Note>
