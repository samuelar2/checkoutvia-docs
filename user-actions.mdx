...
<ResponseField name="key" type="string">
  What is being asked for: `card`, `variant`, `credentials`, `otp`. Branch on this. Omitted
  on actions raised before this field existed.
</ResponseField>
...
`pendingUserAction.key` is that answer: `card`, `variant`, `credentials`, `otp`. It is set
by the engine raising the action, not inferred from the merchant's markup.
...
<Tabs>
  ...
  <Tab title="credentials">
    ...
    <Warning>
    Values you submit here are retained in the action's audit record. Only card-shaped field
    names are redacted; a field literally named `password` is not. See
    [What happens to submitted values](#what-happens-to-submitted-values).
    </Warning>
  </Tab>
...
<Steps>
  <Step title="A redacted copy is written to the action's audit record">
    Field names matching card-shaped patterns — `card`, `cc-num`, `pan`, `cvc`, `cvv`,
    `security code`, `expir` — are replaced with `"[redacted]"` before anything is stored.
    Every other field is stored as sent.
  </Step>
...
<Warning>
**Redaction is by field name, and only card-shaped names are covered.** A `credentials`
action with a field called `password` stores that password in the action's audit record. If
your merchant accounts matter, use throwaway or scoped credentials, and prefer
[buyer sessions](/buyer-sessions) so a password is supplied once rather than every run.
</Warning>
