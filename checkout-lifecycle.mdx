...
There is **no transition back to `running`** after you answer an action.

Submitting a `pendingUserAction` resumes the run, but the status stays
`awaiting_user_action` until the next status-changing event: another action is raised (which
re-enters `awaiting_user_action`) or the checkout goes terminal. During that window the
object reads `status: "awaiting_user_action"` with **no** `pendingUserAction` field.

So the correct client tests are:
