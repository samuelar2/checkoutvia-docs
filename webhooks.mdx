...
A checkout that asks two questions produces **two** `awaiting_user_action` events with
nothing in between, because there is no transition back to `running` after you answer an
action. See [the wart](/checkout-lifecycle#a-wart-worth-knowing).
...
const app = express();
const SECRET = process.env.CKV_WEBHOOK_SECRET;
