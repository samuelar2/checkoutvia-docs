---
title: "Introduction"
description: "Checkoutvia is an HTTP API for agents that buy things. What it does, what it refuses to do, and the shape of the integration."
---

Checkoutvia buys things on the open web on a user's behalf. The caller we build for is an
...
Everything Checkoutvia does happens over HTTP. There is no required SDK, no widget to embed,
no client library you have to wait for. The **web dashboard exists for billing and for issuing
API keys** — and, when you want it, for watching a run go past. It is not where the work
happens, and no capability lives there that the API does not have.
...
<Card title="Use Checkoutvia from an LLM agent" icon="robot" href="/agents">
