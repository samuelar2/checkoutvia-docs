...
<Note>
The API base URL is `https://api.checkoutvia.com`. Every authenticated call takes
...
```bash
export CKV_KEY=ck_test_3sQd7yQ0mHq9pP2rVwXk1tLb
```
...
```bash
export CKV_ID=1f2e3d4c-5b6a-4790-8123-abcdef012345
```
...
<Warning>
**Poll on `pendingUserAction`, not on `status` alone.** Once you answer an action, the
status stays `awaiting_user_action` while the run continues — it only changes again when
the next action is raised or the checkout reaches `succeeded` / `failed` / `cancelled`.
The reliable "the agent needs me" test is `pendingUserAction` being present. See
[Checkout lifecycle](/checkout-lifecycle#a-wart-worth-knowing).
</Warning>
...
```bash
curl -sX POST \
  https://api.checkoutvia.com/v1/checkouts/$CKV_ID/actions/7c1de0a2-9b44-4f0a-b3c1-2a5e8d7f4b60 \
  -H "authorization: Bearer $CKV_KEY" \
  -H 'content-type: application/json' \
...
```bash
curl -sX POST \
  https://api.checkoutvia.com/v1/checkouts/$CKV_ID/actions/9a0b4e51-3c78-4a1d-8e26-51b7cf90a2d4 \
  -H "authorization: Bearer $CKV_KEY" \
  -H 'content-type: application/json' \
  -d '{
    "action": "submit",
    ...
    if (key === "card") {
      checkout = await api(path, {
        ...
        body: JSON.stringify({
          action: "submit",
          values: {
            card_number: process.env.CKV_DEMO_CARD!,
            card_expiry: "12 / 30",
            card_cvc: "123",
          },
        }),
