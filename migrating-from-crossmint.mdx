...
| Reusable **buyer profiles** for identity and shipping | Same. |
...
<AccordionGroup>
  ...
  <Accordion title="The action key is not on the action object">
    Ours is exposed on the timeline instead: the newest `user_action` progress item's
    `data.key`. If your existing code branches on a key field on the action itself, that is
    the one place you will need a shim. See
    [Identifying an action](/user-actions#identifying-an-action).
  </Accordion>

  <Accordion title="No transition back to running after an action resolves">
    Answering an action resumes the run, but the status stays `awaiting_user_action` until
    the next action or a terminal state. Branch on `pendingUserAction` being present, not on
    the status string. See [the wart](/checkout-lifecycle#a-wart-worth-knowing).
  </Accordion>
...
<Steps>
  ...
  <Step title="Fix your action-type branch">
    Branch on `pendingUserAction.key` — `card`, `variant`, `credentials`, `otp`.
  </Step>
