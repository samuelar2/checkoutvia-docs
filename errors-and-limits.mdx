...
| `403` | Account suspended; missing scope; sandbox confinement; live-key request from a sandbox-only account; a key trying to mint a broader key; signup closed. |
| `404` | Not found, or owned by another tenant — the two are not distinguished. Also `Unknown resource` for a path under `/v1` that is not a scoped resource. |
