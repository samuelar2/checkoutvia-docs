---
title: "Use Farthing from an LLM agent"
...
`farthing_create_checkout` accepts an optional `idempotencyKey`. Pass a unique value per
intended purchase and retry with the **same** value: a replay returns the original checkout
instead of buying the item twice. The server will not invent one on your behalf, because only
the caller knows whether a second call means "I retried" or "buy another" — so the tool is
annotated `idempotentHint: false`, and without the key two calls are two purchases. The key
must come from your integration, never from the model.
