...
It does not make a *failed run* retryable: reusing that key returns the failure for good. A real
retry is a new checkout with a new key. Full behaviour, including how the body is hashed and how
keys are namespaced per end user, in [Idempotency](/errors-and-limits#idempotency).
...
and a **new** `Idempotency-Key`, because the old one now points at the failure and will keep
returning it.
...
```json
{
  "mcpServers": {
    "checkoutvia": {
      "command": "npx",
      "args": ["-y", "checkoutvia-mcp"],
      "env": { "CHECKOUTVIA_API_KEY": "ck_test_..." }
    }
  }
}
```
...
| Tool | What it does |
| --- | --- |
| `checkoutvia_list_buyer_profiles` | Call this **first**. Attaching a stored address stops the run pausing to ask for one. |
| `checkoutvia_create_checkout` | Starts the purchase. Returns immediately — nothing has been bought yet. |
| `checkoutvia_get_checkout` | Status, a readable timeline, the question it is waiting on, the receipt. Poll every 10–15s. |
| `checkoutvia_answer_action` | Answers a `variant` or `shipping` question. Refuses the secret ones — see below. |
| `checkoutvia_cancel_checkout` | Stops a run and tears down its browser. |
...
`checkoutvia_create_checkout` is annotated `idempotentHint: false`, honestly: the server sends no
